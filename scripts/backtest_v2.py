#!/usr/bin/env python3
"""
Phase 1 backtest v2: adds separate offense/defense ratings, run/pass split,
rest, weather, and QB availability/injuries on top of the v1 single-EPA model.

Everything below follows the same discipline as v1 (see backtest.py):
ratings and features only ever use information available BEFORE the game
being predicted, and the EPA/feature -> points conversion is refit every
season using only prior seasons (expanding window, no lookahead).

New pieces vs. v1:

1. Off/def split, run/pass split
   Instead of one combined "net EPA/play" rating per team, we track four
   EWMA ratings per team/week: off_pass, off_rush, def_pass_allowed,
   def_rush_allowed. Each matchup then gets a pass_edge and rush_edge that
   properly pit one team's offense against the OTHER team's defense in
   that unit, rather than collapsing everything into one number.

2. Rest
   rest_diff = home_rest - away_rest, straight from games.csv.

3. Weather
   wind (mph) and a dome/indoor flag from games.csv (roof in {dome,closed}).
   Included as direct regressors on the spread -- weather is generally a
   bigger factor for game totals than for who wins by how much, so this is
   partly a test of whether it matters for spread at all.

4. QB availability
   qb_change_{home,away}: 1 if the QB who actually started that game
   (games.csv home_qb_id/away_qb_id) differs from that team's "established"
   starter -- the mode of their last 8 starts, computed leak-free (only
   games strictly before this one). Covers the full 1999-2025 range.

5. Injuries (2009-2025 only, from raw/injuries/injuries_{season}.csv)
   injury_edge = away_out_count - home_out_count, where *_out_count is the
   number of distinct players with a FINAL report_status of "Out" that
   team/week (last report by date_modified). Set to 0 for seasons before
   2009 (no data) -- a documented limitation, not a true zero.
"""
...
# --------------------------------------------------------------------------
# 1. per-game team pass/rush EPA, offense and defense
# --------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--out-dir", default="backtest_v2_out", type=Path)
    ...
    preds[cols].to_csv(args.out_dir / "predictions_v2.csv", index=False)
    (args.out_dir / "summary_v2.json").write_text(json.dumps(summary, indent=2, default=str))
    print(f"\nWrote {args.out_dir/'predictions_v2.csv'} and {args.out_dir/'summary_v2.json'}")
